...
async function loadAll(){q('updated').textContent='refreshing';try{const [o,l]=await Promise.all([get('/operations/overview'),get('/logs/tail')]);const safety=o.safety||{},runtime=o.local_runtime||{},obs=o.observation||{};q('cards').innerHTML=[card('Mode',o.mode||'UNKNOWN'),card('Runtime',runtime.runtime_status||'UNKNOWN',runtime.runtime_status==='PASS'?'ok':'warn'),card('Hermes',runtime.hermes_operational_state||'UNKNOWN',runtime.hermes_status==='OK'?'ok':'warn'),card('Ollama',String(runtime.ollama_available),state(runtime.ollama_available)),card('Execution allowed',String(safety.execution_allowed),state(safety.execution_allowed)),card('Market',obs.market_status||'UNKNOWN',obs.market_status==='OK'?'ok':'warn'),card('Risk gate',obs.risk_gate_status||'UNKNOWN',obs.risk_gate_status==='OK'?'ok':'warn')].join('');q('overview').textContent=pretty(o);q('events').textContent=pretty(l.lines||l);q('updated').textContent='updated '+new Date().toLocaleTimeString()}catch(e){q('events').textContent='Dashboard data error: '+e.message;q('updated').textContent='error'}}
