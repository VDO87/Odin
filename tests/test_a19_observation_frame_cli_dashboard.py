...
class A19ObservationFrameCliDashboardTests(unittest.TestCase):
    def test_cli_observation_frame_returns_json(self):
        env = os.environ.copy()
        env["PYTHONPATH"] = str(Path.cwd() / "src")
        completed = subprocess.run(
            [sys.executable, "-m", "odin.cli", "observation-frame"],
            cwd=Path.cwd(),
            env=env,
            check=True,
            capture_output=True,
        ...
        self.assertEqual(result["component"], "observation_frame")
        self.assertEqual(result["frame_mode"], "OBSERVATION_ONLY")
        self.assertEqual(result["selected_source"], "mt5_feed_mock")
        self.assertIs(result["execution_allowed"], False)

    def test_dashboard_observation_frame_endpoint(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            code, payload = routes.serve("/observation/frame")
